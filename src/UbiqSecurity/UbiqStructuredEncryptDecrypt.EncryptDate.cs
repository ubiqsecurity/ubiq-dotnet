...
namespace UbiqSecurity
{
    public partial class UbiqStructuredEncryptDecrypt
    {
        ...
        public async Task<IEnumerable<DateTime>> EncryptDateForSearchAsync(string datasetName, DateTime plainDateTime, byte[] tweak)
        {
            ...
            {
                var cipherText = await EncryptDatePipelineAsync(dataset, plainDateTime, tweak, keyNumber);
        ...
        private async Task<DateTime> EncryptDatePipelineAsync(FfsRecord dataset, DateTime plainDate, byte[] tweak, int? keyNumber = null)
        {
            ...
            if (plainDate.Kind != DateTimeKind.Utc)
            {
                throw new ArgumentException("plainDate must be UTC", nameof(plainDate));
            }

            var utcPlainDate = plainDate.Date;
            ...
            long daysFromEpoch = (long)(utcPlainDate - dataset.DataTypeConfig.Epoch.Date).TotalDays;

...
using UbiqSecurity.Internals.Structured.Ciphers;
using UbiqSecurity.Internals.Structured.Helpers;
using ICipher = UbiqSecurity.Internals.Structured.Ciphers.ICipher;
...
namespace UbiqSecurity.Internals.Structured
{
    ...
    internal class FF1 : FFX, IFFX
    {
        ...
        public FF1(byte[] key, byte[] twk, long twkmin, long twkmax, int radix)
            : base(key, twk, 1L << 32, twkmin, twkmax, radix, DefaultAlphabet)
        {
            _key = key;
        ...
        public FF1(byte[] key, byte[] twk, long twkmin, long twkmax, int radix, string alphabet)
            : base(key, twk, 1L << 32, twkmin, twkmax, radix, alphabet)
        {
            _key = key;
            _cipher = new SystemCipher();
        }
        ...
        /// </summary>
        /// <param name="x">string to encrypt or decrypt</param>
        /// <param name="tweak">Tweak byte array</param>
        /// <param name="encrypt">Boolean to indicate if <paramref name="x"/> should be encrypted (true) or decrypted (false)</param>
        /// <returns>Encryped or decrypted string</returns>
        ...
        [SuppressMessage("StyleCop.CSharp.NamingRules", "SA1312:Variable names should begin with lower-case letter", Justification = "Variable names match NIST algorithm example")]
        public override string Cipher(string x, byte[] tweak, bool encrypt)
        {
            ...
            // Step 3, 4
            int b = ((int)Math.Ceiling(Math.Log(Radix) / Math.Log(2) * v) + 7) / 8;
            int d = (4 * ((b + 3) / 4)) + 4;
            ...
            if (n < MinimumInputLength || n > MaximumInputLength)
            {
                throw new ArgumentException(UbiqResources.InvalidInputLength);
            }
            else if (tweak.Length < MinimumTweakLength || (MaximumTweakLength > 0 && tweak.Length > MaximumTweakLength))
            {
            ...
            for (var i = 0; i < 10; i++)
            {
                ...
                c = BigIntegerHelper.Parse(B, Radix, Alphabet);
                ...
                c = BigIntegerHelper.Parse(A, Radix, Alphabet);
                ...
                c = BigIntegerHelper.Mod(c, BigInteger.Pow(new BigInteger(Radix), m));

                // Step 6viii
                A = B;

                // Step 6ix
                B = c.ToString(m, Radix, Alphabet);
            }

...
namespace UbiqSecurity.Internals.WebService.Models
{
    internal class FfsRecord
    {
        ...
        [JsonProperty("input_pad_length")]
        public int? InputPadLength { get; set; }
        ...
        [JsonIgnore]
        public int PassthroughSuffixLength => int.Parse(PassthroughRules.Where(x => x.RuleType == PassthroughRuleType.Suffix).FirstOrDefault()?.Value ?? "0", NumberStyles.Integer, CultureInfo.InvariantCulture);

        [JsonIgnore]
        public string PassthroughCharacterSet => PassthroughRules.Where(x => x.RuleType == PassthroughRuleType.Passthrough).FirstOrDefault()?.Value ?? PassthroughCharacters;
    }
}

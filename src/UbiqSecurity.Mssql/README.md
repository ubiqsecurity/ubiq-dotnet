...
CREATE ASSEMBLY ubiq 
FROM 'C:\Ubiq\UbiqSecurity.Mssql.dll' 
...
2. Provide VIEW DEFINITION Permission to the SQL Logins that will be using Ubiq (either by specific user, or role)
```sql
GRANT VIEW DEFINITION TO [your_sql_login];
GO

GRANT VIEW DEFINITION TO [your_sql_role];
GO
```

3. Test the installation by running the following SQL statements, changing the dataset name and plain text accordingly
```sql
### Example Usage

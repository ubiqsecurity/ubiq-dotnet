...
namespace UbiqSecurity.LoadTests
{
    internal class Program
    {
        ...
        private static async Task RunCommand(CommandLineOptions options)
        {
            ...
            else
            {
                ubiqEncryptDecrypt = CryptographyBuilder
                                            .Create()
                                            .WithCredentialsFromFile(options.Credentials, options.Profile)
                                            .BuildStructured();
            }

            using var runner = options.NoTimings ? new TestRunner(ubiqEncryptDecrypt) : new BenchmarkedTestRunner(ubiqEncryptDecrypt);
            var filePaths = isDirectory ? Directory.EnumerateFiles(options.InputFileName, "*.json") : new List<string> { options.InputFileName };
            var timings = await runner.RunAsync(filePaths);

            PrintTimings(timings, options);
        }

        private static void PrintTimings(Timings timings, CommandLineOptions options)
        {
            if (!options.NoTimings)
            { 
    ...
    internal class CommandLineOptions
    {
        ...
        [Option('n', "no-timings", Required = false, HelpText = "Do not measure execution times")]
        public bool NoTimings { get; set; }
    }
}

...
{
    public class UbiqStructuredEncryptDecryptTests : IClassFixture<UbiqStructuredEncryptDecryptFixture>
    {
        private readonly UbiqStructuredEncryptDecryptFixture _fixture;

        public UbiqStructuredEncryptDecryptTests(UbiqStructuredEncryptDecryptFixture fixture)
        {
            _fixture = fixture;
        }
        ...
        public async Task EncryptAsync_ValidInput_ReturnsExpectedCipherText(string dataset, string plainText, string cipherText)
        {
            ...
            var actualCipherText = await sut.InferredEncryptAsync(dataset, plainText);

            Assert.Equal(cipherText, actualCipherText);

            var actualPlainText = await sut.InferredDecryptAsync(dataset, cipherText);
        ...
        public async Task EncryptForSearchAsync_ValidDataset_ReturnedArrayOfCiphersContainsExpectedCipher(string datasetName, string originalPlainText)
        {
            ...
            {
                var plainText = await sut.DecryptAsync(datasetName, cipher);
        ...
        public void GetCopyOfUsage_NoBillingEventInQueue_ReturnsEmptyUsageArray()
        {
            ...
            var sut = CryptographyBuilder.Create().WithCredentials(_fixture.UbiqCredentials).BuildStructured();

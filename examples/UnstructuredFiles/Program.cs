...
namespace UnstructuredFiles
{
    public class Program
    {
        private static async Task<int> Main(string[] args)
        {
            ...
            {
                Console.WriteLine($"{nameof(UnstructuredFiles)} exception: {ex}");
                return 1;       // non-zero means failure
            }
            finally
            {
                Console.WriteLine($"{nameof(UnstructuredFiles)}: done.");
            }
        }
        ...
        private static async Task RunCommand(CommandLineOptions options)
        {
            ...
            if (options.Simple)
            {
                if (options.Encrypt)
                {
                    await SimpleEncryptionAsync(options.InputFile, options.OutputFile, cryptoBuilder);
                }
                else
                {
                    await SimpleDecryptionAsync(options.InputFile, options.OutputFile, cryptoBuilder);
                }
            }
            else
            {
                if (options.Encrypt)
                {
                    await PiecewiseEncryptionAsync(options.InputFile, options.OutputFile, cryptoBuilder);
                }
                else
                {
                    await PiecewiseDecryptionAsync(options.InputFile, options.OutputFile, cryptoBuilder);
                }
            }
        }

        private static async Task SimpleEncryptionAsync(string inFile, string outFile, CryptographyBuilder cryptoBuilder)
        {
            using var ubiq = cryptoBuilder.BuildUnstructuredEncrypt(1);

            var plainBytes = File.ReadAllBytes(inFile);

            var cipherBytes = await ubiq.EncryptAsync(plainBytes);

            File.WriteAllBytes(outFile, cipherBytes);
        }

        private static async Task SimpleDecryptionAsync(string inFile, string outFile, CryptographyBuilder cryptoBuilder)
        {
            using var ubiq = cryptoBuilder.BuildUnstructuredDecrypt();

            var cipherBytes = File.ReadAllBytes(inFile);

            var plainBytes = await ubiq.DecryptAsync(cipherBytes);

            File.WriteAllBytes(outFile, plainBytes);
        }

        private static async Task PiecewiseEncryptionAsync(string inFile, string outFile, CryptographyBuilder cryptoBuilder)
        {
            using var plainStream = new FileStream(inFile, FileMode.Open);
            using var cipherStream = new FileStream(outFile, FileMode.Create);
            using var ubiqEncrypt = cryptoBuilder.BuildUnstructuredEncrypt(1);
        ...
        private static async Task PiecewiseDecryptionAsync(string inFile, string outFile, CryptographyBuilder cryptoBuilder)
        {
            using var cipherStream = new FileStream(inFile, FileMode.Open);
            using var plainStream = new FileStream(outFile, FileMode.Create);
            using var ubiqDecrypt = cryptoBuilder.BuildUnstructuredDecrypt();

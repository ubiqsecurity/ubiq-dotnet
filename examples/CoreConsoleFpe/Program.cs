...
namespace UbiqConsole
{
	class Program
	{
		...
		{
			var cipherText = await UbiqFPEEncryptDecrypt.EncryptAsync(ubiqCredentials, options.EncryptText, options.FfsName);
		...
		{
			var plainText = await UbiqFPEEncryptDecrypt.DecryptAsync(ubiqCredentials, options.DecryptText, options.FfsName);
